...
export default function ViewWeek({ mission }: { mission?: string }) {
...
const ColHeader = ({ day, index }: { day: string, index: number }) => {
  ...
  const startOfWeek = dayjs(date).startOf('isoWeek'); // Lundi de la semaine actuelle

  return (
    <div className="h-14 text-xs uppercase flex flex-col items-center justify-center gap-1">
      {day}{"."}
      <div className="text-xl font-light">
        <div className={`leading-3 rounded-full h-8 w-8 flex items-center justify-center ${startOfWeek.add(index, 'day').format('YYYY-MM-DD') === dayjs().format('YYYY-MM-DD') ? 'bg-secondary' : 'bg-transparent'}`}>
